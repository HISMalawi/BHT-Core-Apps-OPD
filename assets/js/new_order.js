    ...
    <tbody>
      <tr>
        ...
        <td id="ordering-clinician">
          ${user_given_name+' '+user_family_name}
        </td>
      </tr>
      <tr>
...
function submitLabParameters(parameters, url, returnToFunction) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 ) {
      if ( (this.status == 201 || this.status == 200)) {
        ...
        var message = "Error " + this.status + ". There was an error sending lab order to IBLIS (Lab system)";
        labError(message);
...
function labError(message) {
  alertify.alert('Lab order error', message, function(){ redirectToLabOrder() });
}
...
function postOrderToLims(orders){
  ...
  submitLabParameters(lab_details, '/emr_lims_interface', 'redirectToLabOrder');
